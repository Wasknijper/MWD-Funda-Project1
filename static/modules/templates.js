var templates = (function(){
	var templates = {};
	templates.house = new t('<article id="{{=house.Id}}" class="object" style="background-image: url(\'{{=img}}\')"><article id="{{=house.Id}}-detail" class="object-detail"><a href="#detail/{{=house.Id}}"><section><h1>{{=house.Adres}}</h1> <h2>€ {{=price}} k.k.</h2></section></a><section class="info">Close</section></article></arcticle>');
	templates.loading = new t('Loading...');
	...
	templates.list = new t('<a role="close" href="#discover">&#x274c;</a>{{@list}}<article><a href="{{=_val.URL}}"><img src="{{=_val.FotoLarge}}" alt="Foto van {{=_val.Adres}}"><h1>{{=_val.Adres}}</h1>{{=_val.PrijsGeformatteerdHtml}}</a></article>{{/@list}}')
	return templates; 
}());
